...
def _read_payload(
    ...
    if object_id == Obj.BORDER_GATE:
        if subid == 1000:  # квестовые врата HotA
            ...
            return
        if subid == 1001:
            # Могила HotA (спрайт avxgravw.def). Восемнадцать байт: две
            # четвёрки из 0xFF, затем единица, два байта и четыре нуля.
            # Длина измерена на четырнадцати могилах, назначение полей не
            # установлено.
            reader.bytes_(18)
            return
