...
def natural_cap_template(t, terrain):
    ...
    if terrain == Terrain.LAVA:
        return name.startswith((b'avlm',b'avllv',b'avldead',b'avldlog'))
    if terrain == Terrain.SNOW:
        return name.startswith((b'avlm',b'avlr',b'avlsntr',b'avldlog'))
...
def place_barriers(m, assets, islands, reefs, lanes, occupied, seed, themes):
    ...
    multicell = 0
    for key in [i.key for i in islands if not i.z and themes[i.key][1] in (Terrain.SAND, Terrain.WASTELAND)][:2]:
