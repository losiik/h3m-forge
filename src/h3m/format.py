...
class MapFeatures:
    ...
    @property
    def buildings_mask_bytes(self) -> int:
        """Байт на маску построек города: 41 постройка, шесть байт.

        Одинаково во всех версиях, включая HotA — проверено по исходникам
        (``buildingsBytes = 6`` задаётся для RoE и никогда не переопределяется).

        Здесь была попытка подобрать ширину перебором: семь байт снижали число
        падающих HotA-карт с 53 до 12, и это выглядело правдоподобно — HotA же
        добавил постройки Причала и Фабрики. Но подгонка оказалась ложной: она
        не устраняла причину, а сдвигала расхождение дальше по файлу, где оно
        всплывало на ближайшем монстре. Настоящее отличие HotA-города в другом
        месте и пока не найдено.
        """
        return 6
